...
class Framework:
    def __init__(self):
        ...
        self.pipe_eng = Pipeline(DEF_Q_SIZE)
        self.pipe_timer = Pipeline(DEF_Q_SIZE)
        self.pipe_log = Pipeline(DEF_Q_SIZE)
        ...
        if USE_LOGGING:
            self.log_listener = setup_logging(self.pipe_log)
            self.logger = logging.getLogger("[framework]")

        self.engine_thread = Utils.thread_define("engineThread", self._thread_method)
        self.engine_timer = Utils.thread_define(
            "engineTimer", self._thread_method_timer
        )
